...
class Instance:
    ...
    def station_wait_min(self, station_id: int, arrival_min: float) -> float:
        ...
        center = 18 * 60.0
        half_width = 195.0  # from 15:00 to 21:30
...
def choose_nearest_reachable_station(inst: Instance, current: int, battery_kwh: float) -> Optional[int]:
    candidates = reachable_stations(inst, current, battery_kwh, exclude_current=True)
    if not candidates:
        return None
    return min(candidates, key=lambda s: inst.d(current, s))


def execute_distance_policy(inst: Instance, plan: Sequence[int], max_steps: int = 10000) -> RouteResult:
    result = init_result()
    current = DEPOT_ID
    now = START_TIME_MIN
    ...
    while remaining:
        ...
        if steps > max_steps:
            result.feasible = False
            result.reason = "max_steps exceeded in distance policy"
            break

        target = remaining[0]
        if safe_customer_move(inst, current, target, battery):
            current, now, battery, ok = move_to(inst, result, current, target, now, battery)
            if not ok:
                break
            remaining.pop(0)
        else:
            station = choose_nearest_reachable_station(inst, current, battery)
            if station is None:
                result.feasible = False
                result.reason = f"no reachable station from node {current}"
                break
            current, now, battery, ok = move_to(inst, result, current, station, now, battery)
            if not ok:
                break
    ...
    while result.feasible and current != DEPOT_ID:
        ...
        if steps > max_steps:
            result.feasible = False
            result.reason = "max_steps exceeded during depot return"
            break
        if can_reach(inst, current, DEPOT_ID, battery):
            ...
            if station is None:
                result.feasible = False
                result.reason = f"cannot return: no reachable station from {current}"
                break
            current, now, battery, ok = move_to(inst, result, current, station, now, battery)
            if not ok:
                break
...
def run_one(inst: Instance, seed: int, budget_kind: str, ortools_time_limit: int = 3) -> Tuple[List[int], RouteResult, float]:
    start = time.perf_counter()
    plan = solve_customer_plan_ortools(inst, time_limit_sec=ortools_time_limit)
    result = execute_distance_policy(inst, plan)
...
def run_suite(args: argparse.Namespace) -> None:
    ...
    for n in args.sizes:
        for dist_name in args.dists:
            ...
            for search_seed in args.seeds:
                print(f"Running Alg1 on {inst.name}, seed={search_seed}, budget={args.budget} ...", flush=True)
                plan, res, runtime = run_one(inst, search_seed, args.budget, args.ortools_time_limit)
                row = {
                    "dataset": inst.name,
                    "N": n,
                    "distribution": dist_name.upper(),
                    "seed": search_seed,
                    "algorithm": "alg1_distance_oriented_ortools",
                ...
                print(
                    f"  Alg1: feasible={res.feasible}, T={res.total_time_min:.2f} min, "
                    f"D={res.distance_km:.2f} km, charges={res.charges}, runtime={runtime:.2f}s",
                    flush=True,
                )

    results_path = outdir / "results_alg1.csv"
    ...
    routes_path = outdir / "routes_alg1.csv"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Alg1: distance-oriented EVRP policy with OR-Tools customer ordering.")
